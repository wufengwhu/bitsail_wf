...
public class HdfsPartitionComputer<IN extends Row> extends AbstractPartitionComputer<IN> implements PartitionComputer<IN> {
    ...
    public static final int DUMP_ROW_DYNAMIC_FIELD_INDEX = 4;
    ...
    public Tuple2<LinkedHashMap<String, String>, IN> generatePartitionAndRowData(IN in) {
        String dumpType = jobConf.getUnNecessaryOption(
                FileSystemSinkOptions.HDFS_DUMP_TYPE, HDFS_DUMP_TYPE_JSON);
        ...
        for (PartitionInfo partitionInfo : partitionKeys) {
            if (PartitionType.DYNAMIC.equals(partitionInfo.getType())
                ...
                String filed = DEFAULT_PARTITION;

                if (hasDynamicPartition && HDFS_SUPPORT_DYNAMIC_PARTITIONS.contains(jobConf.getUnNecessaryOption(
                        FileSystemSinkOptions.HDFS_DUMP_TYPE, HDFS_DUMP_TYPE_JSON))) {
                    filed = (String) in.getField(DUMP_ROW_DYNAMIC_FIELD_INDEX);
                }
