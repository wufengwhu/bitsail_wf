...
public interface BaseMessageQueueReaderOptions {
  ...
  ConfigOption<Boolean> IS_DECRYPT =
          key(READER_PREFIX + "is_decrypt")
                  .defaultValue(false);

  ConfigOption<Boolean> IS_UNPACK =
          key(READER_PREFIX + "is_unpack")
                  .defaultValue(false);

  ConfigOption<Boolean> TRANSFORMER_APPID_CHECK =
          key(READER_PREFIX + "transformer_appid_check")
                  .defaultValue(false);
